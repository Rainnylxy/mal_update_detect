...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Extract slice transition chains that reach Core Attack Chain, "
            "with no Full Attack Chain in the timeline."
        )
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--out_dir",
        default="./core_transition_outputs/from_pairs_prediction",
    ...
    text_lines: List[str] = []
    text_lines.append("# Slice Chains Reaching Core Attack Chain (No Full)")
    ...
    for (repo, slice_id), seq in sorted(timelines.items(), key=lambda x: (x[0][0], x[0][1])):
        if any(state == LABEL_F for _, _, state in seq):
            continue
        core_positions = [i for i, (_, _, s) in enumerate(seq) if s == LABEL_C]
        ...
        core_commit_num, core_commit, _ = seq[first_core_idx]
        starts_as_core = 1 if first_core_idx == 0 else 0

        timeline_str = " -> ".join(f"{cnum}({chash}):{state}" for cnum, chash, state in seq)
        state_path = " -> ".join(state for _, _, state in seq)
        core_commit_nums = ";".join(str(seq[i][0]) for i in core_positions)
        core_commit_hashes = ";".join(seq[i][1] for i in core_positions)
        ...
        chain_rows.append(
            {
                ...
                "first_core_commit_num": core_commit_num,
                "first_core_commit": core_commit,
                "starts_as_core": starts_as_core,
                "timeline_len": len(seq),
                "core_occurrence_count": len(core_positions),
                "core_commit_nums": core_commit_nums,
                "core_commits": core_commit_hashes,
        ...
        text_lines.append(
            f"- first_core: {core_commit_num} ({core_commit}) | starts_as_core={starts_as_core}"
        )
        text_lines.append(
            f"- core_occurrence_count: {len(core_positions)} | core_commit_nums: {core_commit_nums}"
        )
    ...
    out_csv = os.path.join(args.out_dir, "slice_chains_reaching_core_no_full.csv")
    out_txt = os.path.join(args.out_dir, "slice_chains_reaching_core_no_full.txt")
    ...
    print(f"chains_reaching_core_no_full={len(chain_rows)}")
